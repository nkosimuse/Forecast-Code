...
def getrawdrift(did,filename):
   ...
   # limit data if particular id is requested
   ...
   for k in newData[0].index:
      #dt1=dt.datetime(year,df[2][k],df[3][k],df[4][k],df[5][k],0,0,pytz.utc)
      dt1=datetime(year,newData[2][k],newData[3][k],newData[4][k],newData[5][k],0,0,pytz.utc)
      dtime.append(dt1)
   return newData[8],newData[7],dtime,newData[9]

def getrawdrift_ids(filename):
   '''
   routine to get distinct ids from raw drifter data from ascii files on line
   '''
   url='http://nefsc.noaa.gov/drifter/'+filename
   df=pd.read_csv(url,header=None, delimiter=r"\s+")
   index = np.where(df[2]==int(time.strftime('%m')))[0]
   recentIDS = df.ix[index]
   ids=list(np.unique(recentIDS[0]))
   return ids
...
'''
        ...
        current_time = pytz.utc.localize(datetime.now().replace(hour=0,minute=0,second=0))
        ...
        if self.modelname is "30yr":
            ...
            index2 = index1 + self.hours
            if index2>144:
               index2=144 # JiM Harcode 4 Aug 2014 because forecast is never more than 144 hours
            url = url.format(index1, index2)
